...
df = pd.read_csv("your_data.csv")  # Make sure this file exists locally

filter_columns = ["StudentID", "Gender", "Race", "Age", "GPA", "RentPaidOnTime",
                  "GigIncomeMonthly", "CreditUtilization(%)", "MissedPayments",
                  "StudentLoans", "PartTimeJob", "FinancialLiteracyScore"]

selected_filters = st.multiselect("Select filters to apply to SHAP chart:", filter_columns, default=filter_columns)

if set(selected_filters) != set(filter_columns):
    filtered_df = df[selected_filters + ["CreditScore"]].copy()
    st.warning("Subset selected. Only partial SHAP insights shown.")
else:
    filtered_df = df.copy()

# Prepare data
X = pd.get_dummies(filtered_df.drop(columns=["CreditScore", "StudentID"], errors="ignore"), drop_first=True)
y = df["CreditScore"]
